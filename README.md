...
I used AI mainly for UI generation, building the WinForms views and form layouts. It was also helpful for generating test scaffolding, speeding up the creation of unit and integration tests. When I got stuck on specific bugs (like the DbContext concurrency issue), AI helped me identify the root cause faster.

Everything generated by AI was reviewed carefully to make sure it followed the Clean Architecture layering, respected the patterns I chose (Strategy, State Machine, Repository + UoW), and didn't introduce business logic where it shouldn't be. I also used it to generate methods and algorithms faster, but the architectural decisions, domain modeling, and pattern selection were all mine. AI was a productivity tool, not the architect.
